...
- Moved packaged Actions runner binaries into a package-owned `native/` sibling alongside `dist/`, matching the published binary layout pattern used by `@trebired/logger` and keeping runtime lookup aligned with the packed folder structure.
