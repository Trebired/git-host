...
async function writePublishedPackageJson() {
  ...
  const published = {
    ...packageJson,
    imports: await createPublishedImports(packageJson.imports || {}),
  };
...
async function createPublishedImports(importsMap) {
  const publishedEntries = await Promise.all(
    Object.entries(importsMap).map(async ([alias, target]) => {
      const next = await resolvePublishedImportTarget(String(target));
      return next ? [alias, next] : null;
    }),
  );

  return Object.fromEntries(publishedEntries.filter(Boolean));
}

async function resolvePublishedImportTarget(target) {
  const compiledPath = resolveCompiledTarget(target);
  if (!compiledPath) return null;

  try {
    await fs.access(compiledPath);
    return `./${normalizePath(path.relative(rootDir, compiledPath))}`;
  } catch {
    return null;
  }
}
