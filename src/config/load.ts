...
const GIT_HOST_PROJECT_CONFIG_PATH = ".trebired/git-host/config.ts";
const EMPTY_CONFIG = Object.freeze(normalizeConfig({}));

let cachedConfigs = new Map<string, LoadedGitHostConfig>();

async function loadConfig(projectRoot = process.cwd(), options: LoadGitHostConfigOptions = {}): Promise<LoadedGitHostConfig> {
  ...
  if (!await pathExists(configPath)) throw new Error(`git-host config was not found: ${configPath}`);
  return loadedConfig(configPath, readSourceConfig(await fsPromises.readFile(configPath, "utf8"), configPath));
}

function loadConfigSync(projectRoot = process.cwd(), options: LoadGitHostConfigOptions = {}): LoadedGitHostConfig {
  const root = path.resolve(projectRoot);
  const configPath = options.configPath
  ? path.resolve(root, options.configPath)
  : findConfigSync(options.searchFrom || root, root);
  if (!configPath) return handleMissingConfig(options);
  if (!fs.existsSync(configPath)) throw new Error(`git-host config was not found: ${configPath}`);
  return loadedConfig(configPath, readSourceConfig(fs.readFileSync(configPath, "utf8"), configPath));
}

function loadCachedConfigSync(projectRoot = process.cwd()): NormalizedGitHostConfig {
  const root = path.resolve(projectRoot);
...
function resetConfigCacheForTests(): void {
  cachedConfigs = new Map<string, LoadedGitHostConfig>();
}
...
async function findConfig(startDir = process.cwd(), boundaryDir?: string): Promise<string|null> {
  ...
  for (;; ) {
    const candidate = path.join(current, GIT_HOST_PROJECT_CONFIG_PATH);
...
function handleMissingConfig(options: LoadGitHostConfigOptions): LoadedGitHostConfig {
  if (options.defaultIfMissing === false) throw new Error("git-host config was not found");
  return missingConfig();
}

function missingConfig(): LoadedGitHostConfig {
  return { config: EMPTY_CONFIG, configPath: null, dependencies: [] };
}

function loadedConfig(configPath: string, config: GitHostConfig): LoadedGitHostConfig {
  return {
...
function readSourceConfig(source: string, configPath: string): GitHostConfig {
  const candidate = runConfigSource(source, configPath);
  if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) {
    throw new Error(`git-host config must default-export an object: ${configPath}`);
  }
  return candidate as GitHostConfig;
}
...
function runConfigSource(source: string, configPath: string): unknown {
  try {
    ...
    throw new Error(`git-host config failed to load: ${configPath}: ${reason}`);
  }
}
...
function toRuntimeConfigSource(source: string, configPath: string): string {
  ...
  if (runtimeSource === withoutImports || /\bexport\b/u.test(runtimeSource)) {
    throw new Error(`git-host config only supports a default export: ${configPath}`);
  }
