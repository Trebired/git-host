import { parseGitApiRoute } from "#5f2400e3636a";
...
function parseGitForgeApiRoute(
  pathnameInput: unknown,
  basePathInput: unknown,
): GitForgeApiRoute | null {
  ...
  if (isOverviewAction(action))
  return buildOverviewRoute(action, repositoryKey, segments.length);
  if (action === "actions") return buildActionsRoute(repositoryKey, segments);
  if (action === "releases")
  return segments.length === 3
  ? buildCollectionRoute(action, repositoryKey, segments.length)
  : buildReleaseRoute(repositoryKey, segments);
  if (action === "forks")
  return segments.length === 3
  ? buildCollectionRoute(action, repositoryKey, segments.length)
  : buildForkRoute(repositoryKey, segments);
  if (isCollectionAction(action))
  return buildCollectionRoute(action, repositoryKey, segments.length);
  return null;
}
...
function buildCollectionRoute(
  ...
  if (length !== 3) return null;
  if (action === "forks")
  return { action: "forks", repositoryKey, resource: "fork" };
  if (action === "releases")
  return { action: "releases", repositoryKey, resource: "release" };
  return {
...
function buildReleaseRoute(
  ...
  if (segments.length === 4) {
    ...
    return releaseId
    ? { action: "release", releaseId, repositoryKey, resource: "release" }
    : null;
  }
...
function buildForkRoute(
  ...
  if (segments.length === 5 && segments[4] === "sync") {
    ...
    return forkId
    ? { action: "fork_sync", forkId, repositoryKey, resource: "fork" }
    : null;
  }
  return null;
}

function buildActionsRoute(
  repositoryKey: string,
  segments: string[],
): GitForgeApiRoute | null {
