...
function readWorkflowRunFilters(
  searchParams: URLSearchParams,
): GitForgeWorkflowRunFilters {
  ...
  return {
    ...
    ...(status
      ? {
        ...
        .split(",")
        .map((entry) => text(entry))
        .filter(Boolean) as GitForgeWorkflowRunFilters["status"],
      }
      : {}),
    ...(triggerKind
      ? {
        ...
        .split(",")
        .map((entry) => text(entry))
        .filter(Boolean) as GitForgeWorkflowRunFilters["triggerKind"],
      }
      : {}),
...
function readWorkflowRunEventFilters(
  searchParams: URLSearchParams,
): GitForgeWorkflowRunEventFilters {
  return {
    afterSequence: searchParams.has("afterSequence")
    ? Number(searchParams.get("afterSequence")) || 0
    : undefined,
...
function readWorkflowRunJobFilters(
  searchParams: URLSearchParams,
): GitForgeWorkflowRunJobFilters {
  const status = text(searchParams.get("status"));
  return {
    ...
    ...(status
      ? {
        ...
        .split(",")
        .map((entry) => text(entry))
        .filter(Boolean) as GitForgeWorkflowRunJobFilters["status"],
      }
      : {}),
  };
}

function readWorkflowRunStepFilters(
  searchParams: URLSearchParams,
): GitForgeWorkflowRunStepFilters {
  const status = text(searchParams.get("status"));
  return {
    ...
    ...(status
      ? {
        ...
        .split(",")
        .map((entry) => text(entry))
        .filter(Boolean) as GitForgeWorkflowRunStepFilters["status"],
      }
      : {}),
  };
}

function readWorkflowRunArtifactFilters(
  searchParams: URLSearchParams,
): GitForgeWorkflowRunArtifactFilters {
  return {
    jobRunId: text(searchParams.get("jobRunId")),
