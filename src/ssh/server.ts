...
import {
  createGitSshRuntime,
  validateCreateGitSshServerOptions,
...
function rejectSshAuthentication(
...
async function authenticateSshConnection(
  ...
  if (
    ...
    rejectSshAuthentication(options, runtime, {
  ...
  const auth = normalizeAuthenticationResult(
  ...
  if (
    !usableKey ||
      usableKey instanceof Error ||
      text(usableKey.type) !== text(ctx.key.algo)
  ) {
    rejectSshAuthentication(options, runtime, {
        identity: auth.identity,
        message: "SSH public key validation failed.",
        outcome: "auth_rejected",
        remoteAddress,
  ...
  if (
    ctx.signature &&
      usableKey.verify(ctx.blob, ctx.signature, ctx.hashAlgo) !== true
  ) {
    rejectSshAuthentication(options, runtime, {
        identity: auth.identity,
...
function writeSshAuditEvent(
  ...
  void Promise.resolve(
    options.activity.recordSshAuditEvent(event as any),
  ).catch (() => {});
}

function pipeGitProcessToChannel(
...
async function handleGitSshExec(
  ...
  const resolved = await resolveRepositoryResult(
    options,
    requested.repositoryKey,
    {
  ...
  const authz = normalizeAuthorizationResult(
    options.authorize
  ...
  child.on("error", (error: any) => {
      rejectSshAuthentication(options, runtime, {
          command: requested.command,
          identity: auth.identity,
          message: error?.message ? String(error.message) : "Git process failed.",
          outcome: "failed",
          remoteAddress: auth.remoteAddress,
          remoteUser: authz.remoteUser || auth.remoteUser,
          repository: resolved.repository,
      ...
      writeGitSshFailure(
        channel,
        error?.message ? error.message : "Git process failed.",
      );
  });
}
