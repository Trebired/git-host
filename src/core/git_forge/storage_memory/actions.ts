...
function matchesWorkflowRunFilters(
  entry: GitForgeWorkflowRun,
  filters: GitForgeWorkflowRunFilters = {},
) {
  ...
  return false;
  if (text(filters.actor) && text(entry.created_by) !== text(filters.actor))
  return false;
  if (text(filters.branch) && text(entry.branch) !== text(filters.branch))
  return false;
  if (text(filters.ref) && text(entry.ref) !== text(filters.ref)) return false;
  if (
    text(filters.workflowId) &&
      text(entry.workflow_id) !== text(filters.workflowId)
  )
  return false;
  ...
  if (
    statuses.length &&
      !statuses.map((value) => text(value)).includes(text(entry.status))
  )
  return false;
  const triggers = Array.isArray(filters.triggerKind)
  ? filters.triggerKind
  : filters.triggerKind
  ? [filters.triggerKind]
  : [];
  if (
    triggers.length &&
      !triggers.map((value) => text(value)).includes(text(entry.trigger_kind))
  )
  return false;
  if (
    text(filters.createdAfter) &&
      text(entry.created_at) < text(filters.createdAfter)
  )
  return false;
  if (
    text(filters.createdBefore) &&
      text(entry.created_at) > text(filters.createdBefore)
  )
  return false;
  return true;
}

function sortNewestWorkflowRecords<T extends{created_at:string;id:string}>(entries: T[]) {
...
function sortIndexedWorkflowRecords<T extends{id:string;index:number}>(entries: T[]) {
  return Array.from(entries).sort(
    (left, right) =>
    left.index - right.index || text(left.id).localeCompare(text(right.id)),
  );
}
...
function createRunMethods(state: InMemoryGitForgeState) {
  return {
    ...
    async listWorkflowRuns(
      ...
      return sortNewestWorkflowRecords(
        Array.from(runMap(state, repositoryId).values()).filter((entry) =>
          matchesWorkflowRunFilters(entry, filters),
        ),
      );
    },
    ...
    async updateWorkflowRun(
      repositoryId: string,
      runId: string,
      input: Partial<GitForgeWorkflowRun>,
    ) {
...
function createRunJobMethods(state: InMemoryGitForgeState) {
  return {
    ...
    async listWorkflowRunJobs(
      runId: string,
      filters: GitForgeWorkflowRunJobFilters = {},
    ) {
      const statuses = Array.isArray(filters.status)
      ? filters.status
      : filters.status
      ? [filters.status]
      : [];
      return sortIndexedWorkflowRecords(
        Array.from(runJobMap(state, runId).values()).filter(
          (entry) =>
          (!text(filters.jobId) ||
              text(entry.job_id) === text(filters.jobId)) &&
            (!statuses.length ||
              statuses
            .map((value) => text(value))
            .includes(text(entry.status))),
        ),
      );
    },
    ...
    async updateWorkflowRunJob(
      runId: string,
      jobRunId: string,
      input: Partial<GitForgeWorkflowRunJob>,
    ) {
...
function createRunStepMethods(state: InMemoryGitForgeState) {
  return {
    ...
    async listWorkflowRunSteps(
      runId: string,
      filters: GitForgeWorkflowRunStepFilters = {},
    ) {
      const statuses = Array.isArray(filters.status)
      ? filters.status
      : filters.status
      ...
      : [];
      return sortIndexedWorkflowRecords(
        Array.from(runStepMap(state, runId).values()).filter(
          (entry) =>
          (!text(filters.jobRunId) ||
              text(entry.job_run_id) === text(filters.jobRunId)) &&
            (!statuses.length ||
              statuses
            .map((value) => text(value))
            .includes(text(entry.status))),
    ...
    async updateWorkflowRunStep(
      runId: string,
      stepId: string,
      input: Partial<GitForgeWorkflowRunStep>,
    ) {
...
function createRunArtifactMethods(state: InMemoryGitForgeState) {
  return {
    ...
    async listWorkflowRunArtifacts(
      runId: string,
      filters: GitForgeWorkflowRunArtifactFilters = {},
    ) {
      return sortNewestWorkflowRecords(
        Array.from(runArtifactMap(state, runId).values()).filter(
          (entry) =>
          (!text(filters.jobRunId) ||
              text(entry.job_run_id) === text(filters.jobRunId)) &&
...
function createInMemoryActionsStorage(
  state: InMemoryGitForgeState,
): GitForgeActionsStorage {
  return {
