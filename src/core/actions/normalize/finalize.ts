...
function assertSupportedUses(
  step: GitForgeWorkflowJobStep,
  definitionPath: string,
) {
  if (!step.uses) return;
  if (SUPPORTED_USES.includes(step.uses as(typeof SUPPORTED_USES)[number]))
  return;
  throw new GitHostError(
    "forge_invalid_workflow_definition",
    `Workflow definition "${definitionPath}" uses unsupported action "${step.uses}".`,
    {
      definitionPath,
      uses: step.uses,
    },
  ...
  for (const job of jobs) {
    for (const need of job.needs || []) {
      if (!known.has(need)) {
        throw new GitHostError(
          "forge_invalid_workflow_definition",
          `Workflow definition "${definitionPath}" references unknown job "${need}" in needs.`,
          {
            definitionPath,
            jobId: job.id,
            need,
...
function buildJobsFromLegacySteps(
...
function firstTriggerKind(
  triggers: GitForgeWorkflowTriggers | undefined,
