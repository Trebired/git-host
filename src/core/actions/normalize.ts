...
function normalizeTriggers(value: unknown, legacyTrigger: string, legacySource?: { branches?: string[]; tags?: string[] }): GitForgeWorkflowTriggers | undefined {
  ...
  if (legacyTrigger === "push") {
    return { push: normalizePushTrigger({ branches: legacySource?.branches, tags: legacySource?.tags }) };
  }
  if (legacyTrigger === "manual") {
...
function validateJobs(definitionPath: string, jobs: GitForgeWorkflowJob[]) {
  ...
  for (const job of jobs) {
    ...
    for (const step of job.steps) {
      assertSupportedUses(step, definitionPath);
    }
  }
}
