...
import type { GitForgeWorkflow } from "#14021226ec9b";
...
function normalizeWorkflowRecord(
  ...
  if (!raw || typeof raw !== "object") {
    throw new GitHostError(
      ...
      `Workflow definition "${definitionPath}" must contain an object root.`,
      {
        definitionPath,
        repositoryId,
      },
  ...
  const schema =
  jobs.length || record.on != null
  ? ("gha-subset-v1"as const)
  : ("legacy-shell-v1"as const);
  const normalizedJobs = jobs.length
  ? jobs
  : buildJobsFromLegacySteps(legacySteps);
  if (!normalizedJobs.length) {
    throw new GitHostError(
      "forge_invalid_workflow_definition",
      `Workflow definition "${definitionPath}" must define at least one runnable job or step.`,
      {
        definitionPath,
        repositoryId,
      },
  ...
  const name = text(
    record.name,
    path.basename(definitionPath, path.extname(definitionPath)),
  );
  return {
    ...
    schema,
    slug: slugify(
      text(record.slug, name) ||
        path.basename(definitionPath, path.extname(definitionPath)),
    ),
    source: legacySource,
    steps:
    schema === "legacy-shell-v1"
    ? legacySteps
    : compatibilitySteps(normalizedJobs),
    supported_uses: Array.from(SUPPORTED_USES),
    trigger: firstTriggerKind(triggers, text(record.trigger)),
  };
}

export { SUPPORTED_USES, normalizeEnv, normalizeWorkflowRecord, slugify };
