...
type GitForgeRuntimeContext = {
  ...
  readOverview(
    repositoryId: string,
    actorId?: string,
  ): Promise<GitForgeRepositoryOverview>;
  readRequiredFork(
    forkRepositoryId: string,
  ): Promise<
  Awaited<ReturnType<CreateGitForgeOptions["storage"]["forks"]["readFork"]>>
  >;
  readRequiredRelease(
    repositoryId: string,
    releaseId: string,
  ...
  resolveReleaseAssetLinkInternal(
    repositoryId: string,
    release: GitForgeRelease,
...
function createReadSocialState(options: CreateGitForgeOptions) {
  return async(
    ...
    const [stars, watchers, viewerHasStarred, viewerIsWatching] =
    await Promise.all([
        options.storage.social.listStars
        ? options.storage.social.listStars(repositoryId)
        : Promise.resolve([]),
        options.storage.social.listWatchers
        ? options.storage.social.listWatchers(repositoryId)
        : Promise.resolve([]),
        actorId
        ? options.storage.social.viewerHasStarred(repositoryId, actorId)
        : Promise.resolve(false),
        actorId
        ? options.storage.social.viewerIsWatching(repositoryId, actorId)
        : Promise.resolve(false),
    ]);
    return {
      repository_id: repositoryId,
...
function createReadOverview(
  ...
  return async(
    ...
    const sortedReleases = Array.from(releases).sort((left, right) =>
...
function createReadRequiredRelease(options: CreateGitForgeOptions) {
  return async(
    ...
    const release = await options.storage.releases.readRelease(
      repositoryId,
      releaseId,
    );
    if (!release) {
...
function readRequiredReleaseAsset(
  release: GitForgeRelease,
  assetId: string,
): GitForgeReleaseAsset {
  const asset = release.assets.find(
    (entry) => text(entry.id) === text(assetId),
  );
  if (!asset) {
    throw new GitHostError(
      "forge_resource_not_found",
      `Release asset "${assetId}" was not found in release "${release.id}".`,
      {
        assetId,
...
function createResolveReleaseAssetLinkInternal(options: CreateGitForgeOptions) {
  return async(
    ...
    const repositoryPath = encodeURIComponent(text(repositoryKey, repositoryId));
    const releasePath = encodeURIComponent(release.id);
    const assetPath = encodeURIComponent(asset.id);
    const href = text(
      ...
      text(
        asset.download_url,
        `/repositories/${repositoryPath}/releases/${releasePath}/assets/${assetPath}`,
      ),
    );
    return {
      asset_id: asset.id,
...
function createReadRequiredFork(options: CreateGitForgeOptions) {
  return async(forkRepositoryId: string) => {
    ...
    if (!fork) {
      throw new GitHostError(
        "forge_resource_not_found",
        `Fork "${forkRepositoryId}" was not found.`,
        {
          forkRepositoryId,
        },
      );
...
function createGitForgeContext(
  options: CreateGitForgeOptions,
): GitForgeRuntimeContext {
  ...
  return {
    ...
    logGroup,
    options,
    readOverview: createReadOverview(
      options,
      activityRecorder,
      readSocialState,
    ),
    ...
    recordActivity,
    resolveReleaseAssetLinkInternal:
    createResolveReleaseAssetLinkInternal(options),
    verbose,
  };
}
...
function createReleaseSeed(
  repositoryId: string,
  actor: GitForgeActor,
  input: CreateGitForgeOptions["storage"]["releases"] extends never
  ? never
  : any,
  tagName: string,
  targetRef: string,
  ...
  return {
    ...
    ? null
    : text(input.publishedAt, createdAt),
    repository_id: repositoryId,
    tag_name: tagName,
