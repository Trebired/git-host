...
> {
  return {
    async starRepository(repositoryId, input) {
      ...
      await context.options.storage.social.setStar(
        repositoryId,
        actor.id,
        true,
      );
    ...
    async watchRepository(repositoryId, input) {
      const actor = assertActor(input.actor);
      await context.options.storage.social.setWatching(
        repositoryId,
        actor.id,
        true,
      );
