...
function requireActions(context: GitForgeRuntimeContext, message: string) {
  if (!context.actions)
  throw new GitHostError("forge_actions_not_configured", message);
  return context.actions;
}

function createBaseMethods(
...
> {
  return {
    ...
    async listWorkflows(
      ...
      return context.actions
      ? await context.actions.listWorkflows(repositoryId, filters)
      : [];
    },
    async readWorkflow(repositoryId: string, workflowId: string) {
      return await requireActions(
        context,
        "Actions storage is required to read workflows.",
      ).readWorkflow(repositoryId, workflowId);
    },
    async runWorkflow(
      repositoryId: string,
      workflowId: string,
      ...
      return await requireActions(
        context,
        "Actions storage is required to run workflows.",
      ).runWorkflow(repositoryId, workflowId, {
          ...input,
    ...
    async cancelWorkflowRun(
      repositoryId: string,
      runId: string,
      input: CancelGitForgeWorkflowRunInput,
    ) {
      return await requireActions(
        context,
        "Actions storage is required to cancel workflow runs.",
      ).cancelWorkflowRun(repositoryId, runId, assertActor(input.actor));
    },
    async listWorkflowRuns(
      ...
      return context.actions
      ? await context.actions.listWorkflowRuns(repositoryId, filters)
      : [];
    },
    async readWorkflowRun(repositoryId: string, runId: string) {
      return await requireActions(
        context,
        "Actions storage is required to read workflow runs.",
      ).readWorkflowRun(repositoryId, runId);
    },
    async listWorkflowRunSteps(
      ...
      return context.actions
      ? await context.actions.listWorkflowRunSteps(
        repositoryId,
        runId,
        filters,
      )
    ...
    async listWorkflowRunJobs(
      ...
      return context.actions?.listWorkflowRunJobs
      ? await context.actions.listWorkflowRunJobs(
        repositoryId,
        runId,
        filters,
      )
    ...
    async listWorkflowRunArtifacts(
      ...
      return context.actions?.listWorkflowRunArtifacts
      ? await context.actions.listWorkflowRunArtifacts(
        repositoryId,
        runId,
        filters,
      )
    ...
    async listWorkflowRunEvents(
      ...
      return context.actions
      ? await context.actions.listWorkflowRunEvents(
        repositoryId,
        runId,
        filters,
      )
    ...
    subscribeWorkflowRun(repositoryId: string, runId: string, listener) {
      return context.actions
      ? context.actions.subscribeWorkflowRun(repositoryId, runId, listener)
      : { close: noop };
    },
