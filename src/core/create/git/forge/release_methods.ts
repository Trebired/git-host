...
async function normalizeReleaseAssets(
  context: GitForgeRuntimeContext,
  repositoryId: string,
  assets: unknown,
) {
  if (!context.options.releaseAssetStore?.normalizeAssets)
  return Array.isArray(assets) ? assets : [];
  return await context.options.releaseAssetStore.normalizeAssets(
    repositoryId,
...
async function readReleaseAssetDownload(
  ...
  ? await context.options.releaseAssetStore.openAssetDownload({
      asset,
      release,
      repositoryId,
      repositoryKey,
...
|"openReleaseAsset"
|"readRelease"
|"resolveReleaseAssetLink"
|"updateRelease"
> {
  return {
    async listReleases(repositoryId: string) {
      const releases =
      await context.options.storage.releases.listReleases(repositoryId);
      return Array.from(releases).sort((left, right) =>
        text(right.published_at || right.created_at).localeCompare(
          text(left.published_at || left.created_at),
        ),
      );
    },
    ...
    async resolveReleaseAssetLink(
      repositoryId: string,
      releaseId: string,
      assetId: string,
      input = {},
    ) {
      const release = await context.readRequiredRelease(
        repositoryId,
        releaseId,
      );
      const asset = context.readRequiredReleaseAsset(release, assetId);
      return await context.resolveReleaseAssetLinkInternal(
        repositoryId,
        release,
        asset,
        text(input.repositoryKey),
      );
    },
    async openReleaseAsset(
      repositoryId: string,
      releaseId: string,
      assetId: string,
      input = {},
      ...
      const download = await readReleaseAssetDownload(
        context,
        repositoryId,
        release,
        asset,
      ...
      if (!download) {
        const link = await context.resolveReleaseAssetLinkInternal(
          repositoryId,
          release,
          asset,
          repositoryKey,
      ...
      return {
        ...
        content_type: text(
          ...
          text(asset.content_type, "application/octet-stream"),
        ),
        file_name: text(
          download.file_name,
    ...
    async createRelease(
      ...
      if (!existingTagName && !input.createTag)
      throw new GitHostError(
        "forge_invalid_input",
        "A release requires either existingTagName or createTag.",
        { repositoryId },
      );
      const tagState = await ensureReleaseTag(
        context,
        repositoryId,
        actor,
        input,
      );
      const assets = await normalizeReleaseAssets(
        context,
        repositoryId,
        input.assets,
      );
      const release = createReleaseSeed(
        repositoryId,
        actor,
        input,
        tagState.tagName,
      ...
      const created =
      await context.options.storage.releases.createRelease(release);
      await context.recordActivity(repositoryId, actor, "release.create", {
    ...
    async updateRelease(
      ...
      const updated = await context.options.storage.releases.updateRelease(
        ...
        releaseId,
        {
          ...
          ...(input.publishedAt !== undefined
            ? {
              published_at:
              input.publishedAt === null ? null : text(input.publishedAt),
            }
            : {}),
      ...
      await context.recordActivity(repositoryId, actor, "release.update", {
    ...
    async deleteRelease(
      repositoryId: string,
      releaseId: string,
      input: DeleteGitForgeReleaseInput,
    ) {
      const actor = assertActor(input.actor);
      const existing = await context.readRequiredRelease(
        repositoryId,
        releaseId,
      );
      const deleted = await context.options.storage.releases.deleteRelease(
        repositoryId,
        releaseId,
      );
      if (!deleted)
      ...
      await context.options.gitHost.deleteTag(repositoryId, {
          name: existing.tag_name,
      });
      await context.recordActivity(repositoryId, actor, "release.delete", {
          release_id: releaseId,
...
async function ensureReleaseTag(
  ...
  const tagDetail = await context.options.gitHost.readTag(
    repositoryId,
    tagName,
  );
  return {
