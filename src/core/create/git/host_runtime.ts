...
function validateCreateGitHostOptions(options: CreateGitHostOptions) {
  if (!options || typeof options.resolveRepository !== "function") {
    throw new TypeError(
      "createGitHost() requires a resolveRepository() function.",
    );
  }
}
...
function createGitHostRuntime(options: CreateGitHostOptions): GitHostRuntime {
  ...
  return {
    ...
    logger,
    managedExcludeHeader: text(
      options.managedExcludeHeader,
      DEFAULT_MANAGED_EXCLUDE_HEADER,
    ),
    managedExcludePatterns: normalizeManagedExcludePatterns(
      options.managedExcludePatterns,
    ),
    options,
    verbose: options.verbose === true,
  };
  ...
  return async(repositoryId: string): Promise<GitRepositoryHandle> => {
    ...
    if (!resolved) {
      throw new GitHostError(
        ...
        `Repository "${text(repositoryId)}" was not found.`,
        {
  ...
  return async(
    ...
    try {
      ...
      throw toGitHostError(
        error,
        "git_command_failed",
        "Failed to read repository summary.",
      );
    }
  };
}

async function cloneIntoRepository(
  ...
  if (runtime.verbose)
  runtime.logger.info(runtime.logGroup, "cloning repository", {
  ...
  if (!isDirectoryEmpty(repository.path)) {
    throw new GitHostError(
      ...
      "Repository clone target is not empty.",
      {
        path: repository.path,
        repositoryId: repository.id,
      },
  ...
  if (cloneRes.ok) return true;
  runtime.logger.error(runtime.logGroup, "repository clone failed", {
      path: repository.path,
      repositoryId: repository.id,
      stderr: text(cloneRes.stderr),
  });
  throw new GitHostError(
    "git_command_failed",
    text(cloneRes.stderr, "Failed to clone repository."),
    {
      args: ["clone"],
      repositoryId: repository.id,
    },
...
async function initializeRepository(
  ...
  if (runtime.verbose)
  runtime.logger.info(runtime.logGroup, "initializing repository", {
      path: repository.path,
      repositoryId: repository.id,
  });
  const initRes = await initRepository(
    repository.path,
    text(ensureOptions.initialBranch, DEFAULT_BRANCH),
  );
  if (!initRes.ok) {
    runtime.logger.error(runtime.logGroup, "repository initialization failed", {
        path: repository.path,
        repositoryId: repository.id,
        stderr: text(initRes.stderr),
    });
    throw new GitHostError(
      "git_command_failed",
      text(initRes.stderr, "Failed to initialize repository."),
      {
        args: ["init"],
        repositoryId: repository.id,
      },
  ...
  runtime.logger.error(runtime.logGroup, "initial repository commit failed", {
      path: repository.path,
      repositoryId: repository.id,
      stderr: text(commitRes.stderr),
  });
  throw new GitHostError(
    "git_command_failed",
    text(commitRes.stderr, "Failed to create the initial repository commit."),
    {
      args: ["commit"],
      repositoryId: repository.id,
    },
...
async function ensureHostedRepository(
  runtime: GitHostRuntime,
  repository: GitRepositoryHandle,
  ensureOptions: EnsureRepositoryOptions,
) {
  ...
  if (!hostedConfigRes.ok) {
    runtime.logger.error(
      runtime.logGroup,
      "hosted repository configuration failed",
      {
        path: repository.path,
        repositoryId: repository.id,
        stderr: text(hostedConfigRes.stderr),
      },
    );
    throw new GitHostError(
      "git_command_failed",
      text(
        hostedConfigRes.stderr,
        "Failed to configure the repository for hosted Git access.",
      ),
      {
        repositoryId: repository.id,
      },
...
function createEnsureRepositoryInner(
  ...
  return async(
    ...
    if (
      !(await repositoryExists(repository.path)) &&
        !(await cloneIntoRepository(runtime, repository, ensureOptions))
    ) {
  ...
  return async(
    ...
    if (!(await repositoryExists(repository.path))) {
      throw new GitHostError(
        ...
        `Repository "${repository.id}" is not initialized.`,
        {
          path: repository.path,
          repositoryId: repository.id,
        },
    ...
    return await readSummaryForRepository(
      repository,
      summaryOptions.commitLimit,
    );
  };
}
...
function createMethodContext(
  ...
  const ensureRepositoryInner = createEnsureRepositoryInner(
    runtime,
    resolveRepository,
  );
  return {
