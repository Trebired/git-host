...
import type { resolveLogger } from "#5a29135e56c1";
...
function warnForUnsafeRunnerOptions(
  ...
  if (options?.environment?.inheritProcessEnv) {
    log.warn(
      "actions.runner",
      "actions.environment.inheritProcessEnv is enabled: the entire host process environment is exposed to every workflow step and is " +
  ...
  if (
    ...
    log.warn(
      "actions.runner",
      "the local Actions runner is executing as root with no actions.localRunner.uid drop or beforeSpawn sandbox: workflow steps run as " +
