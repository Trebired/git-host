...
import { readTreeEntryBlob } from "./shared.js";
...
async function readRepositoryTreeEntries(
  ...
  if (treePath) {
    const fallbackRes = await runGitBuffer(
      ["ls-tree", "-z", "-l", ref, "--", treePath],
      { cwd: repository.path },
    );
  ...
  throw new GitHostError(
    "git_command_failed",
    text(treeRes.stderr, "Failed to read repository tree."),
    {
      path: treePath,
      ref,
...
async function readRepositoryLinguistInput(
  ...
  for (let index = 0; index < blobEntries.length; index += 16) {
    const blobs = await Promise.all(
      ...
      .map(async(entry) => await readTreeEntryBlob(repository, ref, entry)),
    );
...
function normalizeLinguistLanguage(
  ...
  value: {
    bytes?: number;
    count?: number;
    lines?: { content?: number; total?: number };
  },
  repositoryMetadata: Record<
  string,
  { color?: string; parent?: string; type?: string }
  >,
): GitRepositoryLinguistLanguage {
  ...
  return {
    ...
    lines: {
      content: Number(value?.lines?.content) || 0,
      total: Number(value?.lines?.total) || 0,
    },
...
function normalizeLinguistResults(
  ...
  return {
    ...
    files: {
      ...
      lines: {
        content: Number(result.files?.lines?.content) || 0,
        total: Number(result.files?.lines?.total) || 0,
      },
      results: Object.fromEntries(
        Object.entries(result.files?.results || {}).map(
          ([filePath, language]) => [
            text(filePath),
    ...
    languages: {
      ...
      lines: {
        content: Number(result.languages?.lines?.content) || 0,
        total: Number(result.languages?.lines?.total) || 0,
      },
      results: Object.fromEntries(
        Object.entries(result.languages?.results || {}).map(
          ([language, value]) => [
            text(language),
            normalizeLinguistLanguage(
              language,
              value || {},
              repositoryMetadata,
            ),
    ...
    unknown: {
      ...
      extensions: Object.fromEntries(
        Object.entries(result.unknown?.extensions || {}).map(
          ([extension, bytes]) => [text(extension), Number(bytes) || 0],
        ),
      ),
      filenames: Object.fromEntries(
        Object.entries(result.unknown?.filenames || {}).map(
          ([fileName, bytes]) => [text(fileName), Number(bytes) || 0],
        ),
      ),
      lines: {
        content: Number(result.unknown?.lines?.content) || 0,
        total: Number(result.unknown?.lines?.total) || 0,
      },
    },
  };
}
...
async function readRepositoryLinguist(
  ...
  try {
    ...
    target = await resolveCommitForRef(
      repository.path,
      target.ref,
      "Repository ref does not exist.",
    );
    await progress.emit("listing_tree", {
        commit: target.commit,
        ref: target.ref,
    });
    const linguistInput = await readRepositoryLinguistInput(
      repository,
      target.ref,
      progress.emit,
    );
    ...
    const normalized = normalizeLinguistResults(
      await analyseRawContent(linguistInput.input, { offline: true }),
      target,
    );
    ...
    .emit("failed", {
        ...
        error: {
          code: isGitHostError(error) ? error.code : "internal_error",
          message:
          error instanceof Error ? error.message : "Linguist scan failed.",
        },
        ref: target.ref,
    })
    .catch (() => undefined);
...
export { readRepositoryLinguist, readRepositoryTreeEntries };
