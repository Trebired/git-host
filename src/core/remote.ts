...
import { assertRepositoryReady, buildGitEnv, runGit } from "./run_git.js";

const REMOTE_USERNAME_ENV = "GIT_HOST_REMOTE_USERNAME";
const REMOTE_PASSWORD_ENV = "GIT_HOST_REMOTE_PASSWORD";
const REMOTE_CREDENTIAL_HELPER = [
  "!f() { ",
  'if [ "$1" = get ]; then ',
  'test -n "$GIT_HOST_REMOTE_USERNAME" && echo "username=$GIT_HOST_REMOTE_USERNAME"; ',
  'test -n "$GIT_HOST_REMOTE_PASSWORD" && echo "password=$GIT_HOST_REMOTE_PASSWORD"; ',
  "fi; ",
  "}; f",
].join("");

function withRemoteUrlOverrideArgs(
  remoteNameInput: unknown,
  remoteUrlInput: unknown,
): string[] {
...
function normalizeRemoteCredentialEnv(
  credentialsInput: GitRemoteCredentials | undefined,
) {
...
function withRemoteCredentialArgs(
  credentialsInput: GitRemoteCredentials | undefined,
): string[] {
...
function withHttpHeaderArgs(
  headersInput: Record<string, string>|undefined,
): string[] {
  const headers =
  headersInput && typeof headersInput === "object" ? headersInput : {};
  const args: string[] = [];
  ...
  for (const [name, value] of Object.entries(headers)) {
    ...
    if (
      ...
      throw new GitHostError(
        "git_command_failed",
        "HTTP headers must not contain newline characters.",
        {
          header: headerName,
        },
      );
...
function buildRemoteGitEnv(
  ...
  const credentialEnv =
  normalizeRemoteCredentialEnv(options.remoteCredentials) || {};
...
async function currentRepositoryBranch(
  ...
  const fallbackRes = await runGit(["rev-parse", "--abbrev-ref", "HEAD"], {
      cwd: repository.path,
  });
...
async function fetchRepository(
  ...
  if (!fetchRes.ok) {
    throw new GitHostError(
      "git_command_failed",
      text(fetchRes.stderr, "Failed to fetch repository remote."),
      {
...
async function pullRepository(
  ...
  const remoteName = text(options.remote, "origin");
  const branchName =
  text(options.branch) || (await currentRepositoryBranch(repository));
  if (!branchName) {
    throw new GitHostError(
      "git_command_failed",
      "Current repository branch is required.",
      {
        repositoryId: repository.id,
      },
    );
  ...
  if (!pullRes.ok) {
    throw new GitHostError(
      "git_command_failed",
      text(pullRes.stderr, "Failed to pull repository changes."),
      {
        repositoryId: repository.id,
        remote: remoteName,
        branch: branchName,
...
async function pushRepository(
  ...
  if (!pushRes.ok) {
    throw new GitHostError(
      "git_command_failed",
      text(pushRes.stderr, "Failed to push repository changes."),
      {
        repositoryId: repository.id,
        remote: remoteName,
        branch: branchName,
