...
function routeOperation(route: GitForgeApiRoute, method: string): GitForgeOperation {
  ...
  switch (route.action) {
    ...
    case "action":
      return method === "PATCH" ? "update" : "read";
    case "action_runs":
...
async function runWorkflowCollectionAction(
  ...
  if (body._method === "POST") {
    return await options.forge.createWorkflow(repositoryId, {
      actor: actor as GitForgeActor,
      enabled: body.enabled !== false,
      env: normalizeEnvRecord(body.env),
      name: text(body.name),
      slug: text(body.slug),
      source: normalizeWorkflowSource(body.source),
      steps: normalizeWorkflowSteps(body.steps),
      trigger: text(body.trigger) as GitForgeWorkflow["trigger"],
    });
  }
...
async function runWorkflowAction(
  ...
  if (body._method === "PATCH") {
    return await options.forge.updateWorkflow(repositoryId, workflowId, {
      actor: actor as GitForgeActor,
      enabled: body.enabled === undefined ? undefined : body.enabled === true,
      env: body.env === undefined ? undefined : normalizeEnvRecord(body.env),
      name: body.name === undefined ? undefined : text(body.name),
      slug: body.slug === undefined ? undefined : text(body.slug),
      source: body.source === undefined ? undefined : normalizeWorkflowSource(body.source),
      steps: body.steps === undefined ? undefined : normalizeWorkflowSteps(body.steps),
      trigger: body.trigger === undefined ? undefined : text(body.trigger) as GitForgeWorkflow["trigger"],
    });
  }
...
function normalizeWorkflowSteps(value: unknown): GitForgeWorkflowStep[] {
  return Array.isArray(value)
    ? value.map((entry) => {
      ...
      return {
        env: normalizeEnvRecord(step.env),
        id: text(step.id),
        kind: "shell" as const,
        name: text(step.name),
        run: text(step.run),
...
function normalizeWorkflowSource(value: unknown) {
  if (!value || typeof value !== "object") return undefined;
  const source = value as Record<string, unknown>;
  return {
    branches: Array.isArray(source.branches) ? source.branches.map((entry) => text(entry)).filter(Boolean) : undefined,
    env: normalizeEnvRecord(source.env),
    tags: Array.isArray(source.tags) ? source.tags.map((entry) => text(entry)).filter(Boolean) : undefined,
  };
}
